...
const Login = ({ onSubmit, onSwitchMode }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!rememberMe) {
      toast.error('You must enable "Remember Me" for login.');
      return;
    }
    setLoading(true);
    try {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {fields().map(({ name, type, placeholder, icon: Icon, isPassword }) => (
          <div key={name} className="relative">
            <div className="flex items-center border rounded-md px-3 py-2 bg-white shadow-sm focus-within:ring-2 focus-within:ring-purple-500">
              <Icon className="text-purple-500 h-5 w-5 mr-2" />
              <input
                type={type}
                name={name}
              ...
              {isPassword && (
                <button
                  ...
                  className="absolute right-3 text-gray-500 hover:text-purple-500 transition-colors"
                  tabIndex={-1}
                >
                  {showPassword ? (
